...
def fit_planes(pcd_iter):
	for topic, msg, ts in pcd_iter:
		pass

# Read bag file
bag_file = bag.Bag("vins_odom_data.bag")

# Create image, mask and pointcloud iterators
im_iter = bag_file.read_messages(['/image'])
mask_iter = bag_file.read_messages(['/mask'])
pcd_iter = bag_file.read_messages(['/vins_estimator/point_cloud'])
