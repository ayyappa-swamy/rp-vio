...
namespace Map3D
{
    class OctoMapEDT
    {
        public:
            ...
            void setMapRange2( Eigen::Vector3d pt , double range );
...
void Map3D::OctoMapEDT::setMapRange2(Eigen::Vector3d pt , double range )
{
    octomap::point3d p = convertToOctomapPt(pt);
...
/*
Eigen::MatrixXd Map3D::OctoMapEDT::GetQueryMatrix(  Eigen::Vector3d start , Eigen::Vector3d end ,  double NumSamples)
{

    int resx = int(  ( end.x() - start.x() )/NumSamples  )
    int resy = int( ( end.y() - start.y() )/NumSamples  )
    int resz = int ( ( end.z() - start.z() )/NumSamples  )

    Eigen::MatrixXd Pts(NumSamples , 4 );
